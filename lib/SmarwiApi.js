...
class SmarwiApi {
  ...
  constructor(address, opts = {}) {
    ...
    // The SMARWI runs on an ESP8266, which accepts only a handful of TCP
    // connections at a time. Overlapping requests exhaust the slots and its web
    // server then stops accepting connections altogether - it answers ping but
    // no longer opens port 80, and only a power cycle brings it back. So every
    // request to a given device goes through this one queue.
    this._queue = Promise.resolve();
  }

  /** Runs `task` once whatever is already queued for this device has finished. */
  _serialise(task) {
    const result = this._queue.then(task, task);
    this._queue = result.then(() => null, () => null);
    return result;
  }
  ...
  _get(path) {
    return this._serialise(() => this._getNow(path));
  }
      ...
      const req = http.get(
        {
          ...
          // No pooling and no keep-alive: a socket the device is still holding
          // is a slot it cannot give to the next request.
          agent: false,
  ...
  _postFile(path, content, filename = '/afile') {
    ...
    return this._serialise(() => new Promise((resolve, reject) => {
