...
class SmarwiApi {
  ...
  /**
   * Resets the Finetune settings to the factory defaults (`GET /rcfa`).
   * This is what the "Reset to defaults" button of the SMARWI web interface does.
   */
  ...
  static parseStatus(body) {
    ...
    return {
      ...
      // pos: 'c' = closed, 'o' = open
      closed: raw.pos === 'c',
      // `ok:0` is what the device's own web interface shows as "not ready";
      // `ro` flips together with it and is kept for reference.
      ready: raw.ok !== '0',
