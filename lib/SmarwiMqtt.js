...
class SmarwiMqtt extends EventEmitter {
  ...
  constructor({ host, port, username, password, log = () => {} } = {}) {
    ...
    this.log = log;
    this.client = null;
    // deviceId -> what the broker last said about it
    this.seen = new Map();
  }

  /**
   * Devices that announced themselves on the broker. Pairing uses this to
   * offer a SMARWI that is not on Homey's own network.
   */
  getKnownDevices() {
    return [...this.seen.entries()].map(([deviceId, entry]) => ({ deviceId, ...entry }));
  }
  ...
  _onMessage(topic, payload) {
    ...
    const entry = this.seen.get(deviceId) || {};

    if (what === 'online') {
      const online = text.trim() === '1';
      this.seen.set(deviceId, { ...entry, online });
      this.emit('online', { deviceId, online });
      return;
    }

    if (what === 'status') {
      try {
        const status = SmarwiApi.parseStatus(text);
        this.seen.set(deviceId, { ...entry, name: status.name, status });
